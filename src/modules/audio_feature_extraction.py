...
def load_audio(filepath: Path, sr=16000):
    data = librosa.load(str(filepath), sr=sr, mono=True)[0]
    return data
...
def preprocess_audio(audio_data, sr, anim_fps, anim_length):
    ...
    mel_spec_interp = interpolate.griddata(
        ...
        mel_spec,
        ((sr / 200) / anim_fps) * np.arange(anim_length),
